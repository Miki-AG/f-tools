...
function getPidPath(rootDir) {
  return path.join(rootDir, TICKETS_DIR_NAME, PID_FILE);
}

function getLogPath(rootDir) {
  return path.join(rootDir, TICKETS_DIR_NAME, LOG_FILE);
}

function ensureIssuesDir(rootDir, fail) {
  const issuesDir = path.join(rootDir, TICKETS_DIR_NAME);
  if (fs.existsSync(issuesDir)) {
    let stat;
    try {
      stat = fs.statSync(issuesDir);
    } catch (err) {
      fail(`Unable to stat ${TICKETS_DIR_NAME}: ${err.message}`);
    }
    if (!stat.isDirectory()) {
      fail(`${TICKETS_DIR_NAME} exists but is not a directory: ${issuesDir}`);
    }
    return;
  }
  ...
  try {
    ...
    fail(`Unable to create ${TICKETS_DIR_NAME}: ${err.message}`);
  }
}
...
function stopPidWithEscalation(pid, fail) {
  try {
    ...
    fail(`Unable to stop f-report (pid ${pid}): ${err.message}`);
  }
  ...
  if (!waitForProcessExit(pid, STOP_TERM_WAIT_MS)) {
    try {
      ...
      fail(`Unable to force-stop f-report (pid ${pid}) after SIGTERM: ${err.message}`);
    }

    if (!waitForProcessExit(pid, STOP_KILL_WAIT_MS)) {
      fail(`f-report process ${pid} is still running after SIGKILL.`);
    }
  }
}
