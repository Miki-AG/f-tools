...
function startDaemon(options, entryScriptPath, fail, failProminent) {
  ...
  if (current && Number.isFinite(current.pid) && isProcessRunning(current.pid)) {
    ...
    process.stdout.write(
      `f-report already running (pid ${current.pid}). Reused existing instance.\n`
    );
    process.stdout.write(
  ...
  if (Number.isFinite(occupiedPid) && isProcessRunning(occupiedPid)) {
    ...
    failProminent("ANOTHER PROCESS IS ALREADY LISTENING ON THE REQUESTED PORT", [
      ...
      `  2) Start f-report on another port: f-report start --port ${options.port + 1}`,
    ]);
  }
  ...
  while (elapsed < STARTUP_STABILITY_MS) {
    ...
    if (!isProcessRunning(child.pid)) {
      clearDaemonMeta();
      fail(`f-report failed to start. Check log: ${logPath}`);
    }
  }
  ...
  try {
    ...
    clearDaemonMeta();
    fail(`f-report started but failed to attach project: ${err.message}`);
  }

  process.stdout.write(`f-report launch requested (pid ${child.pid})\n`);
  ...
  process.stdout.write("Run `f-report status` to confirm it is running.\n");
}

function stopDaemon(fail) {
  const current = readDaemonMeta();
  if (!current || !Number.isFinite(current.pid)) {
    process.stdout.write("f-report is not running.\n");
    return;
  }
  ...
  clearDaemonMeta();
  process.stdout.write(`Stopped f-report (pid ${current.pid}).\n`);
}
...
function showStatus() {
  ...
  if (!current || !Number.isFinite(current.pid)) {
    process.stdout.write(`f-report status: stopped (attached projects: ${projects.length})\n`);
    return;
  }
  ...
  if (!isProcessRunning(current.pid)) {
    clearDaemonMeta();
    process.stdout.write(
      `f-report status: stopped (stale daemon metadata removed, attached projects: ${projects.length})\n`
    );
    return;
  }
  ...
  if (currentRepoPath) {
    process.stdout.write(
      `f-report status: running (pid ${current.pid}) http://${host}:${port} | attached projects: ${projects.length} | current repo: ${attachedToCurrentRepo ? "attached" : "detached"}\n`
    );
    return;
  }

  process.stdout.write(
    `f-report status: running (pid ${current.pid}) http://${host}:${port} | attached projects: ${projects.length}\n`
  );
}
