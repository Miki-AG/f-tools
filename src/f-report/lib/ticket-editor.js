...
const { STATUS_FILE_NAME, TICKETS_DIR_NAME } = require("./constants");
...
function serializeFrontMatter(frontMatter) {
  ...
  for (const key of orderedKeys) {
    ...
    if (key === "depends_on") {
      lines.push(`depends_on: ${formatLabelsInline(frontMatter.depends_on || [])}`);
      continue;
    }
...
function prepareStatusJsonWithUpdates(statusPath, ticket, updatesValue) {
  ...
  if (fs.existsSync(statusPath)) {
    try {
      ...
      throw new Error(
        `Invalid JSON in ${TICKETS_DIR_NAME}/${STATUS_FILE_NAME}: ${err.message}`
      );
    }
    if (!isObject(parsed)) {
      throw new Error(`${TICKETS_DIR_NAME}/${STATUS_FILE_NAME} must be a JSON object.`);
    }
  }
...
function updateTicketById(rootDir, ticketId, payload) {
  ...
  if (!lookup.tickEnabled) {
    return {
      ok: false,
      code: 404,
      error: "f-ticket is not initialized in this folder (missing ./_TICKETS).",
    };
  }
  if (!lookup.ticket) {
    return {
