          ...
          <CardContent className="space-y-6 pt-6">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Parent</div>
                  <div className="rounded-md border border-border/70 bg-muted/30 px-3 py-2 text-sm">
                    {ticket?.parent || "-"}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Depends On</div>
                  <div className="rounded-md border border-border/70 bg-muted/30 px-3 py-2 text-sm">
                    {ticket?.dependsOn && ticket.dependsOn.length > 0 ? ticket.dependsOn.join(", ") : "-"}
                  </div>
                </div>
...
function _TICKETSFilePath(ticket: TicketSummary | null): string {
  const name = String(ticket?.fileName || "").trim();
  return name ? `_TICKETS/${name}` : "-";
}
