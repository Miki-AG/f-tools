...
declare global {
  interface Window {
    __F_REPORT_CONFIG?: ReportConfig;
    __F_TICKET_CONFIG?: TicketConfig;
    __F_REPORT_BOOTSTRAP__?: Partial<BootstrapConfig>;
  }
}
...
export function readBootstrapConfig(): BootstrapConfig {
  const explicit = window.__F_REPORT_BOOTSTRAP__ || {};
  const report = window.__F_REPORT_CONFIG || {};
  const ticket = window.__F_TICKET_CONFIG || {};
